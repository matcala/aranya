...
async fn main() -> Result<()> {
    init_tracing();
    info!("starting aranya-cosmos-proxy-membera");
    ...
    // Start UDP forwarder
    info!("membera: starting UDP forwarder (client-side proxy)");
    let udp_forwarder = UdpForwarder::new(
        env.membera.cosmos_listen_addr.expect("membera should have COSMOS listen addr"), // Listen for COSMOS commands
        env.membera.cosmos_send_addr.expect("membera should have COSMOS send addr")      // Send forwarded telemetry to COSMOS
    ).await?;
    
    info!("membera: starting UDP traffic forwarding through AQC (as sender)");
    udp_forwarder.start_forwarding_as_sender(chan).await?;

//! An Aranya proxy written in Rust.

use std::{env, path::Path, time::Duration};

use anyhow::Result;
use aranya_cosmos_proxy::{config::create_config, env::EnvVars, tracing::init_tracing};
...
async fn main() -> Result<()> {
    ...
    info!("completed aranya cosmos proxy execution");
...
fn client(device: String, uds_sock: &Path, release: &Path) -> Result<Child> {
    let child = Command::new(release.join(format!("aranya-cosmos-proxy-{:}", device)))
        .kill_on_drop(true)
        .arg("--uds-sock")
        .arg(uds_sock)
        .spawn()?;

...
async fn main() -> Result<()> {
    init_tracing();
    info!("starting aranya-cosmos-proxy-memberb");
    ...
    let AqcPeerChannel::Bidi(chan) = client
        .aqc()
        .receive_channel()
        .await
    ...
    // Start UDP forwarder
    info!("memberb: starting UDP forwarder (server-side proxy)");
    let udp_forwarder = UdpForwarder::new(
        env.memberb.target_listen_addr.expect("memberb should have TARGET listen addr"), // Listen for telemetry from target
        env.memberb.target_send_addr.expect("memberb should have TARGET send addr")      // Forward COSMOS commands to target
    ).await?;
    
    info!("memberb: starting UDP traffic forwarding through AQC (as receiver)");
    udp_forwarder.start_forwarding_as_receiver(chan).await?;
